// Phase 6b step 41: V3 Draft canvas → click Data > Data Library → attach Electra FAQ Library.
...
async function shadowFire(p, predicate) {
  // predicate is a function-string evaluated in-page
  return await p.evaluate((predFn) => {
    ...
    while (queue.length) {
      ...
      try {
        for (const el of Array.from(r.querySelectorAll('a, button, [role="treeitem"], span, div, [role="menuitem"]'))) {
          if (pred(el)) {
...
(async () => {
  ...
  try {
    ...
    await sleep(20000);
    await p.screenshot({ path: path.join(SHOTS, '270-v3-canvas.png'), fullPage: true });

    // Verify V3 Draft state
      ...
      while (queue.length) {
        ...
        try {
          for (const b of Array.from(r.querySelectorAll('button'))) {
            ...
            if (/^Version 3 \(Draft\)/.test(t)) return t.slice(0, 60);
          }
          for (const el of r.querySelectorAll('*')) if (el.shadowRoot) queue.push(el.shadowRoot);
        } catch {}
      }
    ...
    // Click "Data Library" treeitem under Data in Explorer
    const dlClick = await shadowFire(p, (el) => {
      const t = (el.textContent || '').trim();
      return t === 'Data Library' && el.offsetParent !== null;
    });
    console.log('click Data Library:', dlClick);
    await sleep(5000);
    await p.screenshot({ path: path.join(SHOTS, '271-data-library-pane.png'), fullPage: true });

    // Probe: see what controls appear (Add Library button etc.)
    const dlProbe = await p.evaluate(() => {
      ...
      while (queue.length) {
        ...
        try {
          ...
          for (const e of Array.from(r.querySelectorAll('span, div, label'))) {
            ...
            if (/Library|Show Sources|Add|Selected|Available/i.test(t) && t.length < 80
      ...
      out.texts = out.texts.slice(0, 25);
      return out;
    });
    console.log('Data Library pane probe:', JSON.stringify(dlProbe, null, 2));
    fs.writeFileSync(path.join(SHOTS, '271-probe.json'), JSON.stringify(dlProbe, null, 2));
    console.log('done');
  } catch (e) {
    console.error('FATAL', e);
    try { await p.screenshot({ path: path.join(SHOTS, '41-FATAL.png'), fullPage: true }); } catch {}
  } finally {
    await sleep(3000);
    await ctx.close();
  }
