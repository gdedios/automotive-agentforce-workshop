...
### LRN-016 Setup "Settings" pages (NetworkSettings etc.) render the controls in a Visualforce CHILD IFRAME
**Phase:** Ej4 (fresh-org verification)
**What we learned:** `Setup → Digital Experiences → Settings` (`/lightning/setup/NetworkSettings/home`) puts the "Enable Digital Experiences" checkbox inside a child iframe at `/_ui/networks/setup/NetworkSetti...` (a VF-in-Lightning page), NOT the top document and NOT a shadow root. A top-frame `document.querySelectorAll('input[type=checkbox]')` and a shadow-DOM walk BOTH return nothing, even though the checkbox is plainly visible in a screenshot. The checkbox id is `thePage:theForm:setDomainPB:enableNetworkPrefId`.
**Pattern to memorialize:** For any Setup *Settings* page where a shadow-DOM walk finds 0 controls, iterate `page.frames()` and operate on the frame whose URL matches `/_ui/.../setup/...`. Use Playwright frame locators (`frame.locator('#esc\\:aped\\:id')`), not `page.evaluate`. NEVER combine locators from two frames with `.or()` — throws "Locators must belong to the same frame." Generalizes to many legacy Setup settings pages (Network, Session, etc.). Diagnostic move: dump `page.frames()` URLs + per-frame checkbox counts first.

### LRN-017 Fresh-org feature enablement has a BACKEND PROPAGATION LAG behind the Setup toggle
**Phase:** Ej4 (fresh-org verification)
**What we learned:** On a brand-new OrgFarm trial, flipping a Setup toggle (Einstein, Agentforce, Digital Experiences) sets the preference immediately, but the backing API/feature lags minutes before it's usable: `sf agent publish` returned `AgentApiNotFound`/HTTP 404 ~15 min after Agentforce was enabled; `DigitalExperienceConfig` deploy returned "Not available for deploy for this organization" right after the Digital Experiences checkbox committed (verified checked + persisted across reload). Same shape as the Atlas-gate lag at Phase 1.
**Pattern to memorialize:** Distinguish "toggle set" from "feature live." After enabling, EXPECT a lag; don't diagnose a 404/"not available" as a defect or a permission gap (all the permsets can be correct). Verify the toggle actually persisted (re-read after reload), then wait and retry the deploy/publish. Budget a soak window (10-30+ min) for fresh-org enablement before the dependent step. Do NOT spin-retry; schedule the retry.

### LRN-018 Experience Cloud site deploy is provision-then-deploy, AND community-create lags fresh DigExp enablement
**Phase:** Ej4 (fresh-org verification)
**What we learned:** Deploying a `DigitalExperienceConfig` for a NEW LWR site cold fails with "Not available for deploy for this organization" even after Digital Experiences is enabled (the `content/Brand_Center` auto-site confirms DigExp itself is on). The site/community must be created FIRST (`sf community create --template-name "Build Your Own (LWR)" ...`), THEN the `digitalExperiences` bundle deploys onto it (confirms LRN-014 provision-then-deploy). BUT on a freshly-enabled org, `sf community create` itself returns `INSUFFICIENT_ACCESS_OR_READONLY` for minutes-to-tens-of-minutes after DigExp is toggled on — the community-creation infrastructure provisions asynchronously behind the toggle.
**Pattern to memorialize:** Experience Cloud on a fresh org is a THREE-stage gate, each with its own lag: (1) enable Digital Experiences (Setup checkbox in the VF iframe — LRN-016); (2) wait for community-create to become available, then `sf community create` the site; (3) deploy the `digitalExperiences` bundle + Network + DigitalExperienceConfig onto it; (4) `sf community publish`. Budget real soak time between (1) and (2). For a workshop facilitator this is why Ej4 site setup is the longest, most fragile phase — and why it should be tested on a genuinely fresh org well before delivery. `sf community template list` confirms "Build Your Own (LWR)" is the right template name.

### LRN-019 The FIRST Experience Cloud site must be created in the UI; CLI/Metadata-API community-create is gated until then
**Phase:** Ej4 (fresh-org verification)
**What we learned:** After enabling Digital Experiences on a fresh org, `sf community create` returns `INSUFFICIENT_ACCESS_OR_READONLY` and `Network` metadata deploy returns "Not available for deploy for this organization" — NOT a permission-set gap (admin has it) and NOT mere lag (persists 30+ min). With 0 existing Networks there is nothing to deploy onto. Root cause: Salesforce gates programmatic (CLI/Metadata-API) community creation until **at least one site has been created through the Setup UI** (`Setup → Digital Experiences → All Sites → New`). Once one site exists, `sf community create` / Network deploys start working.
**Pattern to memorialize:** Treat "create the first Experience Cloud site" as a **mandatory manual UI step** in any fresh-org workshop (like ADL creation — LRN/CLAUDE rule #8). The workshop install script CANNOT bootstrap the site headlessly on a virgin org. Either the facilitator creates the first site in the UI (then the bundle deploys onto it), or the whole site is authored in the UI. The guide's Ej 4 must call this out explicitly. Automation (Playwright) CAN drive the UI "New site" wizard, but plain `sf project deploy`/`sf community create` cannot do the very first one.
